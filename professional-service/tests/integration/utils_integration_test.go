...
import (
	...
	"github.com/hulkdx/findprofessional-backend-pro/professional-service/internal/data/professionalrepository"
...
func NewTestControllerWithTimeProviderWithUserService(db *pgxpool.Pool, timeProvider utils.TimeProvider, userService user.Service) *professional.Controller {
	controller := professional.NewController(
		professional.NewService(professionalrepository.NewRepository(db, timeProvider)),
		userService,
		timeProvider,
	)
	return controller
